  ...
  head: () => ({
    meta: [
      { title: "Today — Sightline SAT" },
      {
        name: "description",
        content:
          "Your daily SAT focus: a predicted score range, one interleaved session and your vocabulary deck a swipe away.",
      },
      { property: "og:title", content: "Today — Sightline SAT" },
      {
        property: "og:description",
        content: "Predicted score range, today's SAT session and your vocabulary deck.",
      },
    ],
  }),
  component: Home,
...
const pages = ["Today", "Vocabulary"] as const;
            ...
            {pages.map((p, i) => (
              <button
                ...
                className={cn(
                  ...
                  page === i ? "text-foreground" : "text-muted-foreground hover:text-foreground",
                )}
              >
                {p}
              </button>
            ))}
            <span className="ml-auto text-[11px] text-muted-foreground/70">swipe →</span>
          </div>
        </div>
  ...
  return (
    <>
      <header className="flex items-baseline justify-between pt-4">
        ...
        <span className="tnum text-xs text-muted-foreground">{profile.daysToExam} days to SAT</span>
      </header>
      ...
      <section className="pt-7">
        ...
        <p className="mt-1.5 text-[15px] leading-relaxed text-muted-foreground">
          A mixed 12-minute set drawn from your two weakest topics plus one skill about to slip below
